<?php
if (!defined('ABSPATH')) exit;

// ==========================================
// 1. INICIALIZACIÓN (MENÚS Y BADGES)
// ==========================================
add_action('admin_menu', function() {
    global $wpdb;

    // 1. Contar documentos pendientes
    $pending_docs = count(get_users(['meta_key' => 'sms_docs_status', 'meta_value' => 'pending', 'fields' => 'ID']));
    
    // 2. Contar solicitudes de servicio pendientes
    $pending_reqs = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}sms_service_requests WHERE status = 'pending'");

    // 3. Contar Cotizaciones (Leads) Pendientes (NUEVO)
    $pending_leads = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}sms_leads WHERE status = 'pending' AND is_verified = 1");
    
    $total_alerts = $pending_docs + $pending_reqs + $pending_leads;
    
    $menu_label = 'Plataforma B2B';
    // Agregar Badge Rojo si hay tareas
    if ($total_alerts > 0) {
        $menu_label .= " <span class='awaiting-mod count-$total_alerts'><span class='pending-count'>$total_alerts</span></span>";
    }
    
    add_menu_page('Plataforma B2B', $menu_label, 'manage_options', 'sms-b2b', 'sms_render_dashboard', 'dashicons-groups');
});

add_action('admin_init', function() {
    // API & Webhook
    register_setting('sms_opts', 'sms_api_secret');
    register_setting('sms_opts', 'sms_account_id');
    register_setting('sms_opts', 'sms_webhook_secret');
    
    // Configuración Notificaciones
    register_setting('sms_opts', 'sms_admin_phone'); // Alerta Admin
    register_setting('sms_opts', 'sms_msg_delay');   // Retraso Anti-Bloqueo
    
    // Servicios y Botones
    register_setting('sms_opts', 'sms_active_service_pages'); 
    register_setting('sms_opts', 'sms_buttons_config'); 
    
    // Economía
    register_setting('sms_opts', 'sms_product_id'); 
    register_setting('sms_opts', 'sms_credits_qty'); 
    register_setting('sms_opts', 'sms_welcome_bonus'); 
    
    // Legal
    register_setting('sms_opts', 'sms_terms_url'); 
});

// ==========================================
// 2. DASHBOARD PRINCIPAL
// ==========================================
function sms_render_dashboard() {
    $tab = $_GET['tab'] ?? 'leads';
    global $wpdb;

    // Contadores para pestañas
    $pending_docs = count(get_users(['meta_key' => 'sms_docs_status', 'meta_value' => 'pending', 'fields' => 'ID']));
    $pending_leads = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}sms_leads WHERE status = 'pending' AND is_verified = 1");

    ?>
    <div class="wrap">
        <h1>Gestión B2B - Centro de Control</h1>
        
        <?php if($pending_docs > 0 || $pending_leads > 0): ?>
            <div class="notice notice-warning" style="border-left-color: #f0ad4e;">
                <p><strong>⚠️ Tareas Pendientes:</strong> 
                <?php if($pending_leads > 0) echo "$pending_leads cotizaciones nuevas. "; ?>
                <?php if($pending_docs > 0) echo "$pending_docs proveedores por verificar."; ?>
                </p>
            </div>
        <?php endif; ?>

        <h2 class="nav-tab-wrapper">
            <a href="?page=sms-b2b&tab=leads" class="nav-tab <?php echo $tab=='leads'?'nav-tab-active':''; ?>">
                📥 Cotizaciones 
                <?php if($pending_leads > 0) echo "<span class='update-plugins count-$pending_leads'><span class='plugin-count'>$pending_leads</span></span>"; ?>
            </a>
            <a href="?page=sms-b2b&tab=providers" class="nav-tab <?php echo $tab=='providers'?'nav-tab-active':''; ?>">
                👥 Proveedores 
                <?php if($pending_docs > 0) echo "<span class='update-plugins count-$pending_docs'><span class='plugin-count'>$pending_docs</span></span>"; ?>
            </a>
            <a href="?page=sms-b2b&tab=services" class="nav-tab <?php echo $tab=='services'?'nav-tab-active':''; ?>">🔘 Botones y Servicios</a>
            <a href="?page=sms-b2b&tab=requests" class="nav-tab <?php echo $tab=='requests'?'nav-tab-active':''; ?>">🔔 Solicitudes</a>
            <a href="?page=sms-b2b&tab=config" class="nav-tab <?php echo $tab=='config'?'nav-tab-active':''; ?>">⚙️ Configuración</a>
        </h2>
        <div style="background:#fff; padding:20px; border:1px solid #ccc; margin-top:10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
            <?php 
            if($tab == 'leads') sms_tab_leads();
            elseif($tab == 'providers') sms_tab_providers();
            elseif($tab == 'services') sms_tab_services();
            elseif($tab == 'requests') sms_tab_requests();
            else sms_tab_config();
            ?>
        </div>
    </div>
    <?php
}

// ==========================================
// 3. PESTAÑA: COTIZACIONES (LEADS)
// ==========================================
function sms_tab_leads() {
    global $wpdb;
    
    // ACCIONES
    if (isset($_POST['action_lead'])) {
        $lid = intval($_POST['lead_id']);
        
        $new_req = sanitize_textarea_field($_POST['edited_req']);
        $new_quota = intval($_POST['edited_quota']);
        $new_prio = sanitize_text_field($_POST['edited_priority']);
        $new_date = sanitize_text_field($_POST['edited_deadline']);

        // Guardar Cambios
        if ($_POST['action_lead'] == 'save_edit') {
            $wpdb->update("{$wpdb->prefix}sms_leads", 
                ['requirement'=>$new_req, 'max_quotas'=>$new_quota, 'priority'=>$new_prio, 'deadline'=>$new_date], 
                ['id' => $lid]
            );
            echo '<div class="notice notice-success is-dismissible"><p>✅ Datos actualizados.</p></div>';
        }
        
        // Aprobar y Notificar
        if ($_POST['action_lead'] == 'approve') {
            $cost = intval($_POST['lead_cost']);
            $selected_providers = isset($_POST['target_providers']) ? array_map('intval', $_POST['target_providers']) : [];

            $wpdb->update("{$wpdb->prefix}sms_leads", 
                ['status'=>'approved', 'cost_credits'=>$cost, 'requirement'=>$new_req, 'max_quotas'=>$new_quota, 'priority'=>$new_prio, 'deadline'=>$new_date], 
                ['id' => $lid]
            );
            
            // Hook de notificación con lista de seleccionados
            do_action('sms_notify_providers', $lid, $selected_providers);
            
            $count_sent = count($selected_providers);
            echo '<div class="notice notice-success is-dismissible"><p>🚀 Cotización Aprobada. Se enviarán <strong>'.$count_sent.'</strong> notificaciones.</p></div>';
        }

        // Despublicar / Eliminar
        if ($_POST['action_lead'] == 'unapprove') {
            $wpdb->update("{$wpdb->prefix}sms_leads", ['status' => 'pending'], ['id' => $lid]);
            echo '<div class="notice notice-warning is-dismissible"><p>🚫 Ocultada.</p></div>';
        }
        if ($_POST['action_lead'] == 'delete') {
            $wpdb->delete("{$wpdb->prefix}sms_leads", ['id' => $lid]);
            $wpdb->delete("{$wpdb->prefix}sms_lead_unlocks", ['lead_id' => $lid]); 
            echo '<div class="notice notice-error is-dismissible"><p>🗑️ Eliminada.</p></div>';
        }
    }

    // FILTROS
    $filter_status = $_GET['f_status'] ?? 'pending'; // Por defecto ver pendientes
    $where = "WHERE 1=1";
    if($filter_status == 'pending') $where .= " AND status = 'pending'";
    if($filter_status == 'approved') $where .= " AND status = 'approved'";

    $leads = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}sms_leads $where ORDER BY created_at DESC LIMIT 100");
    ?>
    
    <div class="tablenav top">
        <form method="get">
            <input type="hidden" name="page" value="sms-b2b">
            <input type="hidden" name="tab" value="leads">
            <select name="f_status">
                <option value="all" <?php selected($filter_status, 'all'); ?>>Todos</option>
                <option value="pending" <?php selected($filter_status, 'pending'); ?>>Pendientes (Por Aprobar)</option>
                <option value="approved" <?php selected($filter_status, 'approved'); ?>>Publicados</option>
            </select>
            <input type="submit" class="button" value="Filtrar">
        </form>
    </div>

    <table class="widefat striped">
        <thead>
            <tr>
                <th>Estado</th>
                <th>Cliente (Privado)</th>
                <th>Urgencia</th>
                <th>Detalle</th> 
                <th style="width:35%;">Gestión</th>
                <th></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach($leads as $l): 
                $page = get_post($l->service_page_id);
                $service_name = $page ? $page->post_title : '(General)';
                $unlocks = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}sms_lead_unlocks WHERE lead_id={$l->id}");
                $remaining = max(0, $l->max_quotas - $unlocks);
                
                // Buscar Proveedores Aptos
                $eligible_providers = [];
                if ($l->status == 'pending') {
                    $all_users = get_users(['meta_key' => 'sms_approved_services', 'meta_compare' => 'EXISTS']);
                    foreach($all_users as $prov) {
                        $prov_services = get_user_meta($prov->ID, 'sms_approved_services', true);
                        $phone_st = get_user_meta($prov->ID, 'sms_phone_status', true);
                        if (is_array($prov_services) && in_array($l->service_page_id, $prov_services) && $phone_st == 'verified') {
                            $p_name = get_user_meta($prov->ID, 'sms_commercial_name', true) ?: $prov->display_name;
                            $eligible_providers[] = ['id' => $prov->ID, 'name' => $p_name];
                        }
                    }
                }
            ?>
            <tr>
                <td>
                    <?php echo ($l->is_verified) ? '<span style="color:green;">✅ Verif.</span>' : '<span style="color:red;">❌ No Verif.</span>'; ?><br>
                    <strong><?php echo strtoupper($l->status); ?></strong><br>
                    <small><?php echo date_i18n('d/M H:i', strtotime($l->created_at)); ?></small>
                </td>
                <td style="background:#f0f6fc;">
                    <strong><?php echo esc_html($l->client_company ?: 'Persona'); ?></strong><br>
                    👤 <?php echo esc_html($l->client_name); ?><br>
                    📞 <?php echo esc_html($l->client_phone); ?><br>
                    ✉️ <?php echo esc_html($l->client_email); ?>
                </td>
                <td>
                    <?php echo esc_html($l->priority); ?><br>
                    📅 <?php echo $l->deadline ? date('d/M', strtotime($l->deadline)) : '-'; ?>
                </td>
                <td>
                    <strong><?php echo esc_html($service_name); ?></strong>
                    <div style="font-size:11px;">Cupos: <strong><?php echo $unlocks; ?>/<?php echo $l->max_quotas; ?></strong></div>
                </td>
                <td>
                    <form method="post" style="padding:10px; background:#fff; border:1px solid #ddd;">
                        <input type="hidden" name="lead_id" value="<?php echo $l->id; ?>">
                        
                        <textarea name="edited_req" style="width:100%; height:40px; font-size:12px; margin-bottom:5px;"><?php echo esc_textarea($l->requirement); ?></textarea>
                        
                        <div style="display:flex; gap:5px; margin-bottom:5px;">
                            <select name="edited_priority" style="font-size:11px;">
                                <option value="Normal" <?php selected($l->priority, 'Normal'); ?>>Normal</option>
                                <option value="Urgente" <?php selected($l->priority, 'Urgente'); ?>>Urgente</option>
                                <option value="Muy Urgente" <?php selected($l->priority, 'Muy Urgente'); ?>>Muy Urgente</option>
                            </select>
                            <input type="date" name="edited_deadline" value="<?php echo $l->deadline; ?>" style="font-size:11px;">
                        </div>

                        <div style="display:flex; gap:5px; margin-bottom:10px;">
                            <input type="number" name="edited_quota" value="<?php echo $l->max_quotas; ?>" style="width:50px;" min="1">
                            <button type="submit" name="action_lead" value="save_edit" class="button button-small">💾</button>
                        </div>

                        <?php if($l->status == 'pending'): ?>
                            <div style="border-top:1px solid #eee; padding-top:5px;">
                                <div style="font-size:10px; font-weight:bold; margin-bottom:3px;">Notificar a:</div>
                                <div style="max-height:80px; overflow-y:auto; border:1px solid #eee; padding:3px; margin-bottom:5px;">
                                    <label style="display:block; font-size:10px;">
                                        <input type="checkbox" onchange="toggleAll(this)" checked> <strong>Todos</strong>
                                    </label>
                                    <?php foreach($eligible_providers as $ep): ?>
                                        <label style="display:block; font-size:10px;">
                                            <input type="checkbox" name="target_providers[]" value="<?php echo $ep['id']; ?>" checked class="chk-prov"> 
                                            <?php echo esc_html($ep['name']); ?>
                                        </label>
                                    <?php endforeach; ?>
                                </div>
                                <div style="display:flex; justify-content:space-between; align-items:center;">
                                    <input type="number" name="lead_cost" value="10" style="width:50px;" min="1" title="Costo Créditos">
                                    <button type="submit" name="action_lead" value="approve" class="button button-primary button-small">🚀 Aprobar</button>
                                </div>
                            </div>
                        <?php else: ?>
                            <div style="color:green; font-size:11px; text-align:center;">✅ Publicada</div>
                        <?php endif; ?>
                    </form>
                </td>
                <td>
                    <?php if($l->status == 'approved'): ?>
                        <form method="post"><input type="hidden" name="lead_id" value="<?php echo $l->id; ?>"><button type="submit" name="action_lead" value="unapprove" class="button button-small" style="color:orange;">⏸️</button></form>
                    <?php endif; ?>
                    <form method="post" onsubmit="return confirm('¿Eliminar?');" style="margin-top:5px;">
                        <input type="hidden" name="lead_id" value="<?php echo $l->id; ?>">
                        <button type="submit" name="action_lead" value="delete" class="button button-link-delete" style="color:red;"><span class="dashicons dashicons-trash"></span></button>
                    </form>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <script>function toggleAll(src){var c=src.closest('form').querySelectorAll('.chk-prov');for(var i=0;i<c.length;i++)c[i].checked=src.checked;}</script>
    <?php
}

// ==========================================
// 4. PESTAÑA: PROVEEDORES
// ==========================================
function sms_tab_providers() {
    global $wpdb;

    if (isset($_POST['prov_action'])) {
        $uid = intval($_POST['user_id']);
        $phone = get_user_meta($uid, 'sms_whatsapp_notif', true);

        if ($_POST['prov_action'] == 'approve_services') {
            $req = get_user_meta($uid, 'sms_requested_services', true) ?: [];
            update_user_meta($uid, 'sms_approved_services', $req);
            if(function_exists('sms_send_msg') && $phone) sms_send_msg($phone, "✅ *Servicios Aprobados*\nYa puedes recibir cotizaciones.");
            echo '<div class="notice notice-success"><p>Servicios aprobados.</p></div>';
        }
        if ($_POST['prov_action'] == 'approve_docs') {
            update_user_meta($uid, 'sms_docs_status', 'verified');
            if(function_exists('sms_send_msg') && $phone) sms_send_msg($phone, "✅ *Empresa Verificada*\nDocumentos aprobados.");
            echo '<div class="notice notice-success"><p>Documentos aprobados.</p></div>';
        }
        if ($_POST['prov_action'] == 'reject_docs') {
            update_user_meta($uid, 'sms_docs_status', 'rejected');
            delete_user_meta($uid, 'sms_company_docs'); 
            if(function_exists('sms_send_msg') && $phone) sms_send_msg($phone, "❌ *Documentos Rechazados*\nSube imágenes legibles.");
            echo '<div class="notice notice-error"><p>Documentos rechazados.</p></div>';
        }
        if ($_POST['prov_action'] == 'manual_credit') {
             $amt = intval($_POST['credit_amount']);
             $curr = (int) get_user_meta($uid, 'sms_wallet_balance', true);
             update_user_meta($uid, 'sms_wallet_balance', $curr + $amt);
             echo '<div class="notice notice-success"><p>Saldo ajustado.</p></div>';
        }
    }

    $users = get_users(['orderby' => 'ID', 'order' => 'DESC']); 
    ?>
    <h3>Gestión de Proveedores</h3>
    <table class="widefat striped">
        <thead><tr><th>Proveedor</th><th>WhatsApp</th><th>Docs</th><th>Servicios</th><th>Saldo</th></tr></thead>
        <tbody>
            <?php foreach($users as $u): 
                $docs_st = get_user_meta($u->ID, 'sms_docs_status', true);
                $docs = get_user_meta($u->ID, 'sms_company_docs', true);
                $req = get_user_meta($u->ID, 'sms_requested_services', true) ?: [];
                $app = get_user_meta($u->ID, 'sms_approved_services', true) ?: [];
                $pending_servs = array_diff($req, $app);
                $bg = ($pending_servs || $docs_st == 'pending') ? 'background:#fff9e6;' : '';
            ?>
            <tr style="<?php echo $bg; ?>">
                <td>
                    <strong><?php echo get_user_meta($u->ID, 'sms_commercial_name', true) ?: $u->display_name; ?></strong><br>
                    <?php echo $u->user_email; ?>
                </td>
                <td>
                    <?php echo (get_user_meta($u->ID, 'sms_phone_status', true)=='verified') ? '✅' : '❌'; ?><br>
                    <small><?php echo get_user_meta($u->ID, 'sms_whatsapp_notif', true); ?></small>
                </td>
                <td>
                    <?php if($docs_st=='pending'): ?>
                        <strong style="color:orange">Revisar:</strong><br>
                        <?php foreach($docs as $d) echo "<a href='$d' target='_blank'>📄 Doc</a> "; ?>
                        <form method="post" style="margin-top:5px;">
                            <input type="hidden" name="user_id" value="<?php echo $u->ID; ?>">
                            <button type="submit" name="prov_action" value="approve_docs" class="button button-small">✔</button>
                            <button type="submit" name="prov_action" value="reject_docs" class="button button-small" style="color:red;">✖</button>
                        </form>
                    <?php elseif($docs_st=='verified'): echo '<span style="color:green">OK</span>'; 
                    else: echo '-'; endif; ?>
                </td>
                <td>
                    <?php if($pending_servs): ?>
                        <div style="color:red; font-size:10px;">Nuevos servicios:</div>
                        <form method="post">
                            <input type="hidden" name="user_id" value="<?php echo $u->ID; ?>">
                            <button type="submit" name="prov_action" value="approve_services" class="button button-primary button-small">Aprobar</button>
                        </form>
                    <?php else: echo 'Al día'; endif; ?>
                </td>
                <td>
                    <strong><?php echo (int)get_user_meta($u->ID, 'sms_wallet_balance', true); ?></strong>
                    <form method="post" style="display:inline;">
                        <input type="hidden" name="user_id" value="<?php echo $u->ID; ?>">
                        <input type="number" name="credit_amount" style="width:40px;" placeholder="+/-">
                        <button type="submit" name="prov_action" value="manual_credit" class="button button-small">Ok</button>
                    </form>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <?php
}

// ==========================================
// 5. PESTAÑA: BOTONES Y SERVICIOS
// ==========================================
function sms_tab_services() {
    if(isset($_POST['save_config'])) {
        update_option('sms_active_service_pages', $_POST['active_pages'] ?? []);
        $new_buttons = [];
        if(isset($_POST['btn_labels'])) {
            $labels = $_POST['btn_labels']; 
            $quotas = $_POST['btn_quotas']; 
            $page_groups = $_POST['btn_pages_ids']; 
            for($i=0; $i < count($labels); $i++){
                if(!empty(trim($labels[$i]))){
                    $pgs = isset($page_groups[$i]) ? $page_groups[$i] : [];
                    $new_buttons[] = ['label'=>sanitize_text_field($labels[$i]), 'max_quotas'=>intval($quotas[$i]), 'pages'=>array_map('intval', $pgs)];
                }
            }
        }
        update_option('sms_buttons_config', $new_buttons);
        echo '<div class="notice notice-success"><p>Configuración guardada.</p></div>';
    }

    $all_pages = get_pages(['post_status'=>'publish']);
    $active = get_option('sms_active_service_pages', []);
    $btns = get_option('sms_buttons_config', []);
    if(empty($btns)) $btns = [['label'=>'Cotizar', 'max_quotas'=>3, 'pages'=>[]]];
    ?>
    <form method="post">
        <div class="card" style="margin-bottom:20px; padding:15px;">
            <h3>1. Servicios Habilitados</h3>
            <div style="height:150px; overflow-y:scroll; border:1px solid #ddd; padding:10px;">
                <?php foreach($all_pages as $p): ?>
                <label style="display:block;"><input type="checkbox" name="active_pages[]" value="<?php echo $p->ID; ?>" <?php checked(in_array($p->ID, $active)); ?>> <?php echo esc_html($p->post_title); ?></label>
                <?php endforeach; ?>
            </div>
        </div>
        <div class="card" style="padding:15px;">
            <h3>2. Botones Flotantes</h3>
            <div id="buttons_wrapper">
                <?php foreach($btns as $idx => $btn): $btn_pages = $btn['pages'] ?? []; ?>
                <div class="btn-row" style="background:#f9f9f9; border:1px solid #ddd; padding:10px; margin-bottom:10px;">
                    <p>Etiqueta: <input type="text" name="btn_labels[]" value="<?php echo esc_attr($btn['label']); ?>"> Max Cotizaciones: <input type="number" name="btn_quotas[]" value="<?php echo esc_attr($btn['max_quotas']); ?>" style="width:50px;"></p>
                    <select name="btn_pages_ids[<?php echo $idx; ?>][]" multiple style="width:100%; height:100px;">
                        <?php foreach($all_pages as $p): ?>
                        <option value="<?php echo $p->ID; ?>" <?php echo in_array($p->ID, $btn_pages)?'selected':''; ?>><?php echo esc_html($p->post_title); ?></option>
                        <?php endforeach; ?>
                    </select>
                    <button type="button" onclick="this.parentElement.remove()" style="color:red;">Eliminar</button>
                </div>
                <?php endforeach; ?>
            </div>
            <button type="button" class="button" id="add_btn_row">➕ Añadir Botón</button>
            <hr>
            <button type="submit" name="save_config" class="button button-primary">Guardar Cambios</button>
        </div>
    </form>
    <script>
        document.getElementById('add_btn_row').addEventListener('click', function(){
            var w = document.getElementById('buttons_wrapper');
            var c = w.children[0].cloneNode(true);
            var i = w.children.length;
            c.querySelector('input[type=text]').value='';
            c.querySelector('select').name='btn_pages_ids['+i+'][]';
            var o=c.querySelector('select').options; for(var k=0;k<o.length;k++)o[k].selected=false;
            w.appendChild(c);
        });
    </script>
    <?php
}

// ==========================================
// 6. PESTAÑA: SOLICITUDES
// ==========================================
function sms_tab_requests() {
    global $wpdb;
    if(isset($_POST['req_action'])) {
        $wpdb->update("{$wpdb->prefix}sms_service_requests", ['status'=>'completed'], ['id'=>intval($_POST['request_id'])]);
        // Notificar al proveedor
        $prov_id = intval($_POST['prov_id']);
        $phone = get_user_meta($prov_id, 'sms_whatsapp_notif', true);
        if(function_exists('sms_send_msg') && $phone) sms_send_msg($phone, "✅ *Solicitud Atendida*\nLa categoría que pediste ha sido creada.");
        echo '<div class="notice notice-success"><p>Completado.</p></div>';
    }
    $reqs = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}sms_service_requests ORDER BY created_at DESC");
    ?>
    <h3>Solicitudes de Categorías</h3>
    <table class="widefat striped">
        <thead><tr><th>Fecha</th><th>Proveedor</th><th>Solicitud</th><th>Acción</th></tr></thead>
        <tbody>
            <?php foreach($reqs as $r): $u = get_userdata($r->provider_user_id); ?>
            <tr>
                <td><?php echo $r->created_at; ?></td>
                <td><?php echo $u ? $u->display_name : 'ID '.$r->provider_user_id; ?></td>
                <td><?php echo esc_html($r->requested_service); ?></td>
                <td>
                    <?php if($r->status == 'pending'): ?>
                        <form method="post">
                            <input type="hidden" name="request_id" value="<?php echo $r->id; ?>">
                            <input type="hidden" name="prov_id" value="<?php echo $r->provider_user_id; ?>">
                            <button type="submit" name="req_action" value="complete" class="button button-small">✅ Hecho</button>
                        </form>
                    <?php else: echo '✔'; endif; ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <?php
}

// ==========================================
// 7. PESTAÑA: CONFIGURACIÓN
// ==========================================
function sms_tab_config() {
    ?>
    <form method="post" action="options.php">
        <?php settings_fields('sms_opts'); do_settings_sections('sms_opts'); ?>
        <table class="form-table">
            <tr><th>API Secret</th><td><input type="text" name="sms_api_secret" value="<?php echo get_option('sms_api_secret'); ?>" class="regular-text"></td></tr>
            <tr><th>Account ID</th><td><input type="text" name="sms_account_id" value="<?php echo get_option('sms_account_id'); ?>" class="regular-text"></td></tr>
            
            <tr><th colspan="2"><hr><h3>🔔 Alertas & Límites</h3></th></tr>
            <tr><th>Teléfono Admin</th><td><input type="text" name="sms_admin_phone" value="<?php echo get_option('sms_admin_phone'); ?>"> <span class="description">Recibe alertas de nuevos leads.</span></td></tr>
            <tr><th>Retraso Mensajes (seg)</th><td><input type="number" name="sms_msg_delay" value="<?php echo get_option('sms_msg_delay', 0); ?>" class="small-text"> <span class="description">Pausa entre envíos masivos para evitar bloqueo.</span></td></tr>
            
            <tr><th colspan="2"><hr><h3>💰 Créditos</h3></th></tr>
            <tr><th>ID Producto Recarga</th><td><input type="number" name="sms_product_id" value="<?php echo get_option('sms_product_id'); ?>" class="small-text"></td></tr>
            <tr><th>Créditos x Unidad</th><td><input type="number" name="sms_credits_qty" value="<?php echo get_option('sms_credits_qty', 100); ?>" class="small-text"></td></tr>
            <tr><th>Bono Bienvenida</th><td><input type="number" name="sms_welcome_bonus" value="<?php echo get_option('sms_welcome_bonus', 0); ?>" class="small-text"></td></tr>
            
            <tr><th>URL Términos</th><td><input type="text" name="sms_terms_url" value="<?php echo get_option('sms_terms_url'); ?>" class="regular-text"></td></tr>
        </table>
        <?php submit_button(); ?>
    </form>
    <?php
}

// ==========================================
// 8. WOOCOMMERCE CREDITOS
// ==========================================
add_action('woocommerce_order_status_completed', 'sms_add_credits_on_purchase');
function sms_add_credits_on_purchase($order_id) {
    if (get_post_meta($order_id, '_sms_credits_given', true)) return;
    
    $order = wc_get_order($order_id);
    $pid = (int) get_option('sms_product_id');
    $qty_per_unit = (int) get_option('sms_credits_qty', 100);
    $total_credits = 0;

    foreach ($order->get_items() as $item) {
        if ($item->get_product_id() == $pid || $item->get_variation_id() == $pid) {
            $total_credits += ($item->get_quantity() * $qty_per_unit);
        }
    }

    if ($total_credits > 0 && $order->get_user_id()) {
        $uid = $order->get_user_id();
        $curr = (int) get_user_meta($uid, 'sms_wallet_balance', true);
        update_user_meta($uid, 'sms_wallet_balance', $curr + $total_credits);
        update_post_meta($order_id, '_sms_credits_given', 1);
        $order->add_order_note("✅ B2B: Se agregaron $total_credits créditos.");
    }
}
